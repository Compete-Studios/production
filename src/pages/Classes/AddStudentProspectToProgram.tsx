...
import { addAStudentToProgram, addProspectToClass, addProspectToProgram, addStudentToClass, dropProspectFromClass, dropStudentFromClass, searchProspectsBySearchTerm } from '../../functions/api';
import { showMessage } from '../../functions/shared';

export default function AddStudentProspectToProgram({ programId, studentRoster, student = true, updatedPrograms, setUpdatedPrograms }: any) {
    ...
    const handleCheckboxChange = async (e: any, prospect: any) => {
        if (e.target.checked) {
            ...
            try {
                const response = student ? await dropStudentFromClass(prospect.value, programId) : await dropProspectFromClass(prospect.value, programId);
    ...
    const handleAddToClass = async () => {
        ...
        for (let i = 0; i < selectedStudents.length; i++) {
            ...
            try {
                const response = student ? await addAStudentToProgram(classData) : await addProspectToProgram(programId,  selectedStudents[i].value);
